...
for i = 1:numIterations
    ...
    while terminal == false
        ...
        Q(state_index, action) = (Q(state_index, action) + ((alpha)*(delta)*(Z(state_index, action))));

        if next_action == optimal_next_action
            Z(state_index, action) = ((gamma)*(lambda)*Z(state_index, action));
        else
            Z(state_index, action) = 0;
        end
