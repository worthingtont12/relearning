...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%TD(lambda) you're updating v_pi and eligibility trace over the entire state space
...
for i = 1:num_episodes
    ...
    for j = 1:(length(statesFromEpisodes{i})-1)
        ...
        %eligibility trace
        E(state_index) = lambda*gamma*E(state_index) + 1;
        ...
        v = v_pi(state_index) + ((((alpha)*(E(state_index))*((next_reward))) + (gamma)*(next_v) - v_pi(state_index)));
            %save v_pi for s
        v_pi(state_index) = v;
    end
    all_v_pi(:,i) = v_pi;
